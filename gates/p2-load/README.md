...
[`gates/p2-dashboard`](../p2-dashboard/README.md), reads that stream and exits
non-zero when any p99 misses its D16 target.

The shape mirrors the P0 precedent ([`gates/p0-nat-test`](../p0-nat-test/README.md)):
...
the range it covers — `gates/p2-dashboard` prints their unit in its own column.
...
its **own name on purpose**. `gates/p2-dashboard` folds by series name into one
...
methods report, so the gate (`gates/p2-dashboard`) reconstructs percentiles that
...
requires an FDB-enabled `persistd`, `gates/p2-load`, `p2-evidence-verify`, and
`gates/p2-dashboard` binary plus `ORRERY_FDB_CLUSTER_FILE`:

```sh
ORRERY_FDB_CLUSTER_FILE=/path/to/fdb.cluster \
PERSISTD_BIN=target/release/persistd \
P2_LOAD_BIN=gates/p2-load/target/release/p2-load \
P2_DASHBOARD_BIN=gates/p2-dashboard/target/release/p2-dashboard \
...
records into the load JSONL and invokes `gates/p2-dashboard --gate` for all four D16

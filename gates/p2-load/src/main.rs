...
//! page-in). `gates/p2-dashboard --gate` reads that stream and exits non-zero when
...
//! contract documented in `gates/p2-load/README.md` is the delivered telemetry
...
async fn run() -> Result<()> {
    let cli = Cli::parse();
    // Logs on stderr (the gates/p0-nat-test contract): stdout is the JSONL stream.
            ...
            GatewayReply::IntentAck { intent_id, outcome } => {
                let evidence_outcome = match &outcome {
                    ...
                    IntentOutcome::Provisional { .. } => {
                        tracing::error!(
                            intent_id,
                            "intent committed provisionally; gates/p2-load has no evidence shape for it"
                        );
...
mod tests {
    ...
    fn generated_intent_verifies_as_its_connected_issuer() {
        ...
        assert!(
            ...
            "gates/p2-load must sign the canonical preimage with the endpoint identity"
        );

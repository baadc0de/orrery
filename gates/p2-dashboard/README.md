...
telemetry emitted by `gates/p2-load --json` and reports the four D16 latency series
...
`orrery_persist_client::latency` and shared with `gates/p2-load` and persistd. That
is what makes a percentile reported here the same number the rig measured.

The sibling tool is [`gates/p0-dashboard`](../p0-dashboard/README.md): same shape
(read JSONL, compute percentiles, print a pass/fail table, `--gate` exits
non-zero), different numbers.

## What it does

Reads one or more `.jsonl` files from `gates/p2-load --json`. The stream is one JSON
object per line:
...
# binary: gates/p2-dashboard/target/release/p2-dashboard
```
...
The unit tests (`cargo test --manifest-path gates/p2-dashboard/Cargo.toml`) cover

...
//!
//! Aggregates the JSONL telemetry emitted by `p0-nat-test --json`
...
#[derive(Parser)]
#[command(about = "P0 punch-rate dashboard: aggregate p0-nat-test --json soak telemetry")]
struct Cli {
    /// One or more `.jsonl` telemetry files from `p0-nat-test --json`.
    #[arg(required = true)]
    files: Vec<PathBuf>,

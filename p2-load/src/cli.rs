...
//!
//! Mirrors the P0 precedent (`p0-nat-test/src/cli.rs`): one flat `Parser`
...
/// latency series are emitted as JSONL on stdout for `p2-dashboard --gate`.
#[derive(Debug, Parser)]
...
pub struct Cli {
    ...
    /// machine-parseable contract `p2-dashboard` consumes). Tracing logs go to
    /// stderr.
    #[arg(long)]
    pub json: bool,

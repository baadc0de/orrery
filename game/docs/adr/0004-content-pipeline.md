...
- Procurement from **CGTrader and Fab** for kits, greebles, PBR sets and decals, each a licensed provenance input. **Meshes default to the private asset store**; placing an asset in the public repository is an explicit per-asset act under a §1-passing licence. Rationale: no lawyer on the team, assets ship cooked anyway, and a private default conforms to arbitrary licences without per-asset judgement. The private store is a **Hetzner Object Storage bucket** (S3-compatible, content-hash keys, immutable, versioned), accessed by scoped credentials from the pipeline, cook and build, with client fetches going through the distribution path. The public repository holds pipeline code, provenance records and deliberately public assets only; the season-data distribution record must cover client and cook fetches from the private store.

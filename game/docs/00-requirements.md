...
- **G10.2 is chosen on Linux evidence; the Windows number is still owed.** #1069 measured both prongs at N=24 on a quiet box (ABI crossing 20–22 µs, zero drops), but the #920 bands are Windows-defined and no Windows report exists. The choice stands on the frame-cost argument; the Windows run remains a D53 acceptance item.

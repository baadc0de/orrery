...
## Image-to-3D (TRELLIS.2-4B) → Blender render: works

The full TRELLIS.2 stack builds on this box in a user-local conda env (CUDA 12.4 toolkit + gcc 12, since the system has no nvcc and gcc 16): torch 2.6 cu124, flash-attn 2.7.3 (prebuilt; needs `TMPDIR` on the same filesystem as the pip cache), nvdiffrast, nvdiffrec (needs `-L<env>/lib/stubs` for `-lcuda`), CuMesh, FlexGEMM, o-voxel, **transformers pinned `<5`** (the DINOv3 feature extractor walks model internals that 5.x moved). Script: `~/trellis2-build.sh`; runner: `trellis_run.py`; chain: `~/trellis-when-gated.sh`; Blender import-and-render: `blender_render.py`.

**Result on the escort concept (single ¾ image, seed 7):** load 82 s, generate 87 s, export 15 s; **18 MB GLB, 191,640 triangles, textured**; Blender 5.2 headless renders (hero, front, side, top) in `out/trellis/`. Design fidelity to the concept is high: stripe, nose RCS block, canopy, conduits, thrusters, fin and skids all present in 3D; decals smear (the `E-07` is illegible) and the mesh is far above a game budget, so the G12 remesh/retopo and texture-projection steps are real work, not a formality.

**Two licence gates, and how they were handled (G12.8):**
- **DINOv3** (`facebook/dinov3-vitl16-pretrain-lvd1689m`): gated with manual approval, reported to take days. Used an ungated re-upload of the same transformers-format weights that ships Meta's `LICENSE.md`; the DINOv3 licence is recorded as a versioned input of every TRELLIS.2 artifact even though the encoder never ships. The owner's access request is pending; switch `pipeline.json` back to the official repo when it clears.
- **BRIA RMBG-2.0** (`briaai/RMBG-2.0`): gated and non-commercial. Not used: the runner removes the background with u2net (rembg, MIT) and passes RGBA, which the pipeline accepts without its own remover; the loader is stubbed so it is never instantiated.

## Remesh, retopology and mirroring (`retopo.py`)

Pipeline in Blender 5.2 headless: join → centre → **measure the symmetry plane** (mean nearest-neighbour distance of every 7th vertex against its mirror across x=0 and y=0; the craft mirrors across **x** with error 0.0019 vs 0.035 on y) → **voxel remesh** at size/220 (323k tris, 0 non-manifold edges) → retopology → **symmetrize** across the measured plane → smart UV → **Cycles GPU bake** of base colour and tangent normals from the high-poly (selected-to-active, cage 2 % of size) → GLB with 2k textures. 7 s end to end; renders in `out/retopo/`.

**Result:** 191,640 → **24,842 tris**, symmetric, textured, 22 MB GLB with the two 2k PNGs. Symmetrize mirrors decals too (the hull number now reads on both flanks), so in the real pipeline decals are placed *after* symmetry, as a separate layer (G12.11 procurement: decals are inputs).

**Open item: QuadriFlow refuses the escort.** Blender's `quadriflow_remesh` returns CANCELLED with "needs to be manifold and have face normals that point in a consistent direction" on every escort-derived shell (voxel at /220 and /60, octree remesh, OBJ round trip, tri/quad rebuild, both normal orientations, custom normals cleared, ×10 scale, identity matrix), even though the largest shell has every edge on exactly two faces, no degenerate faces, positive signed volume and no ngons; a voxelized sphere and torus pass in the same session. The fallback is **collapse-decimate with symmetry, then tris-to-quads** (24 % quads). True quad topology needs either the cause found in Blender's check or a standalone quad remesher (Instant Meshes) in the Blender-MCP stage; logged as the next spike for G12.9.

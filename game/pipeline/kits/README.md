...
**Railing, round two (owner critique, locks).** The owner's read of the concept beat the reader's five times in one look: the handrail inverted, one brace piece per V-edge instead of two, housings clipping into the posts (aligned on their front instead of their back), base plates turned 90°, the mid-rail-to-post brackets missing. Consequences now in the scripts: `part_sizes` (one size per part id for the whole build) and `count` (runs of instances end to end) in the build list; moves must carry a new absolute centre; human corrections are written into the build list as a `human_critique` stage and the fields they fix are `locked`, which the critic is told about and which the apply step enforces (it refused a handrail re-flip and the removal of the owner's brackets on the very next passes). Scores on the locked build 0.8, 0.8 with 17/21 items good.

Two findings that limit this track: the artist used part 3 at two sizes (0.9 m kick-plate halves, 0.5 m post pieces), so "one size per part" is a rule the concept itself breaks and posts and kick plate cannot both be right; and the critic's edits without item indices (its "add"-style rows) are unusable, so the schema should require `i` on everything but add.

...
def shortlist(z, pool, crop, hint, want):
    ...
    prompt = (f"Contact sheets of untextured kitbash parts, each numbered under its thumbnail (numbers run across sheets, 1..{len(pool)}). Image CROP shows the concept feature to reproduce: "
    ...
    parts = [{"text": prompt}] + ([{"text": "CROP:"}, img(crop["png"])] if crop else [{"text": "CROP unavailable; use the full concept:"}, img(a.concept)])
...
for z in Z["zones"]:
    ...
    crop = crops.get(z["name"]); random.shuffle(c); c.sort(key=lambda x: -x["conf"]); pool = c[:72]; sl_reason = ""
    ...
    prompt = (f"You choose kitbash parts to match concept art. Image 1 is the full concept of a 9 m spacecraft. Image 2 is a CROP of the concept around the feature to reproduce. "
              f"Zone '{z['name']}' at hull region '{z['region']}' reproduces this feature: {z.get('concept_feature','')} (tags {z['tags']}), placed {size}.{hint} "
    ...
    parts += [{"text": "IMAGE 2, CROP:" + ("" if crop["visible"] else " (feature not clearly visible; use the concept's style)")}, img(crop["png"])] if crop else [{"text": "IMAGE 2, CROP: not available; use the full concept."}]
    ...
    try:
        ...
        choices[z["name"]] = {"ranked": order, "reason": v.get("reason", ""), "shortlist_reason": sl_reason, "pool": len(pool), "model": resp.get("modelVersion", a.model), "crop": bool(crop)}

...
| F-E | **Offline fallback indistinguishable from live** (#503 → #508) | After the failed dial the client continued into `ActiveSession::Local` with nothing on screen; the player "flew, aimed and fired against a dead offline world" and misdiagnosed their controls | The scope banner (`LOCAL PRACTICE - NOT CONNECTED TO A CAMPAIGN - NOTHING IS BEING BANKED (dial failed)`, `session_banner_text` in `clients/regolith/src/lib.rs`; reworded and lifted above the join gate by #769) and `session_scope: "local"|"campaign"` on every telemetry envelope |

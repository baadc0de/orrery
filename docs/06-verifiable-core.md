...
- **Static gates.** clippy `disallowed_types` (std `HashMap`/`HashSet`) and `disallowed_methods` (`std::time::*`, `Instant::now`, `thread_rng`, std float transcendentals) on `orrery_core` and the game's core crates; a CI check that `orrery_core` builds without Bevy in its feature graph.

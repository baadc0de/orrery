...
**Demo criterion — met (verified 2026-08-21, #237).** 8 peers on real, heterogeneous NATs (≥4 distinct NAT types, ≥2 ISPs, including the forced-relay peer) form a full mesh and exchange per-tick 60 Hz state datagrams for 30 minutes with zero session drops; the dashboard shows direct-path rate consistent with the ~90% baseline, the relayed peer's traffic flowing, and added relay latency quantified per pair. (60 Hz here is a deliberate transport stress at sim-tick rate; the production replication default is 20 Hz, D16.)
...
- Journal with adaptive group commit (fsync immediately when the disk is idle, ~0.5 ms batching under load; commit < 2 ms server-internal); optional chain replication to one async follower (default on, RPO ≤ ~100 ms for bulk on node loss).
  - **Bounded, as of [D20](adr/0020-journal-retention.md).** The journal had no retention at all: `truncate_before` had no caller, so a node's journal — and the index rebuilt from it at every open — grew with its uptime, at a measured 3.94 µs and ~95 bytes per record. At the gate's own arrival rate that is a 94 GB journal and a 4.3-minute restart after one hour of run time, with no bound and no budget to fail against. The checkpoint scheduler now releases behind the minimum of what its shards have checkpointed and what the chain follower has mirrored, a scan below the floor fails loudly rather than answering short, and D16 gains a `journal_open_ms` budget to measure the result against. One residual is named there and remains: the archive tailer (P6) is still what the event history needs. The other — a follower's own mirror, unbounded because its dedupe cursor was rebuilt from batch zero — is closed by [D23](adr/0023-follower-journal-retention.md), which sends the primary's floor down the chain, seeds the cursor from the durable row it already writes, and makes retention a **mandatory gate clause**: the run fails unless both nodes released, both floors advanced past `0:0`, and every node's `journal_open_ms` came in under D16's 2 000 ms budget. That last clause is not a formality: across three cadence arms on the self-hosted box, the arm whose follower released *nothing* opened its promoted journal in **2 905 ms** — past the budget, after a thirty-second load — against 764 ms and 300 ms for the arms that released ([docs/data/p2-follower-retention-2026-08-20.json](data/p2-follower-retention-2026-08-20.json)). **The gate holds with it on**: four alternating arms on a qualified `c4d-standard-32-lssd` (`fdatasync` p99 0.06 ms) passed 4/4 — `journal_commit_ms` p99 1 ms in every arm — with retention demonstrably active in its two (13 and 17 releases inside the 30-second load phase, the first dropping 233 208 records from the index) and every acknowledged write recovered after the `kill -9`. The same comparison on an unqualified box fails identically in both arms, which is that device's 7 ms `fdatasync` p99 rather than retention.
- FDB 7.3.x checkpointing on the 20 s jittered cadence, immediate on cell quiesce; keyspace as D11 minus `chunk/` (`world/{cell_id}/{entity_id}`, `player/…`, `ledger/…`, `lease/…`). `chunk/` was removed from v1 by [D51](adr/0051-v1-terrain-is-not-durable-state.md).
- Area load: 27-cell FDB range scans + live actor deltas, streamed nearest-first.
- Offline world-seed import tool on the persistd harness (D11): bulk-writes designed content into `world/` rows, mints `PersistId`s, and records a content-version row so later deploys can diff/patch designed content. Implemented as the TOML scenario runner specified in [12-world-seeding.md](12-world-seeding.md). Terrain is not a v1 durable-state import ([D51](adr/0051-v1-terrain-is-not-durable-state.md)).
- The intent *execution* path (signature check → `Ruleset` validation stub → FDB serializable optimistic transaction) without witness attestation, so commit latency is measurable now; attestation arrives in P5. The validation stub is `BaselineIntentValidator` — envelope shape, the one executor-interpreted op, its account binding, and attestation authenticity; every durable invariant is still owed by a linked `Ruleset` ([08-persistence.md](08-persistence.md) §2.2). It is deliberately neither a blanket refusal (which makes `intent_commit_ms` unmeasurable — the state the P2 gate was in, `committed: 0, rejected: 1024`) nor a blanket admission (which makes the measurement meaningless).
- Latency rig: gateway-colocated load generator producing calibrated diff and intent mixes.

**Demo criterion — met (verified 2026-08-21, #239).** With 10k entities across 100+ cells under synthetic load: `kill -9` the entire cluster, restart it, and the world resumes — zero acked intents lost (RPO 0), bulk loss bounded by the journal/replication window, clients (netsplit posture, D12) having queued intents and continued simulating. Measured against D16 targets in-region: journal commit < 2 ms server-internal, client-observed bulk ack p99 < 5 ms, intent commit p99 < 10 ms, area first page-in < 50 ms. **And the journals must be bounded while it happens** ([D23](adr/0023-follower-journal-retention.md)): both nodes must have released behind their floors during the run — the primary behind its checkpoints, the follower behind the floor the primary sends it — and every node's `Journal::open` must come in under D16's 2 000 ms `journal_open_ms` budget. A recovery that is exact on a journal nothing ever released is a recovery measured on the one configuration the deployment does not run.
...
**Those runs predate the retention clause**, which
[D23](adr/0023-follower-journal-retention.md) added to the criterion above on
the same day: they ran at D16's 20 s ± 5 s cadence, where whether a release
fired at all was jitter, and their follower's mirror was pinned by
construction. The durability half they establish is unchanged — the clause
constrains the *configuration* the recovery is measured on, not the recovery —
and the first run to answer the strengthened criterion is the next nightly.
...
**Demo criterion.** A scripted 128-player crowd event (R6 upper bound) in one region: the hot cell is promoted within the hysteresis window, per-peer bandwidth stays within the ≤ 1 Mbps uplink budget and field-host egress within the ≤ 35 Mbps hot-cell budget (D6; the modeled n=128 load is ~25.6+ Mbps, inside budget) throughout, and demotion follows dispersal cleanly. Then the restoration demo, on the settlement tier (v1 has no durable terrain — [D51](adr/0051-v1-terrain-is-not-durable-state.md)): a scripted crowd builds a durable formation in one cell — parked cruisers with hulls and shields topped up, a mined rock pocket, dropped pickups — built in-session and never seeded, so every component write is journaled and archived, and with retention active, so the archive is the only place that history still exists; a griefer on a named `NodeId` destroys and vandalizes it; an operator restores it by computing each entity's pre-grief image from the archive — selected on the server-assigned LSN axis and the gateway-filled `author`, never the client-supplied tick — and applying it forward as attributed `Restore` records appended at the current journal position: nothing rewound, no ledger family written, and the conservation sweep clean before and after because the restore moved no value (the contract: [08-persistence.md](08-persistence.md) §11.1). The plan is inspected before it is applied and must refuse by name what the archive cannot prove — at least one `Refused(PreimageUnavailable)` and one `Held` candidate among a restorable majority — or the demonstration has not shown it can refuse; a re-run of the same plan applies nothing twice. Multi-region: EU-based peers joining a US island get relay/gateway routing that keeps added latency within the measured relay penalty from P0.

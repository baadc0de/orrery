...
Measured on three interleaved before/after runs of the same gate on the same box: `bulk_ack_ms` p99 1 945 / 1 985 / 2 018 ms before, **20 / 30 / 50 ms after**, with 24–25 % of diffs shed and the acked remainder (402 807 – 405 444 of ~540 000) fully recovered by the post-kill verifier. Two honest caveats. `gateway_ingress_queue_ms` gets *worse* (2 530–3 253 ms p99), because the histogram samples every inbound message including the shed ones: after the change it measures the age of the backlog being destroyed rather than the delay being served, and it is no longer a predictor of client-observed latency. And the queue itself is still unbounded — the loop can be stalled by the lease/claim work it runs inline, which is what lets a backlog form at all; bounding that is the next step, not this one.

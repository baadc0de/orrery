...
> end at a claim tick.
>
> Transport landed with the adapter: `orrery_witness::plugin` streams frames
> and claims on `Channel::State` to the witness set and repairs gaps over
> `Channel::Control`. What the streaming costs is settled too — the lane holds
> 20% of the peer upload budget and derives its frame cadence from that share
> ([03-replication.md](03-replication.md) §5.3a), measured at 190 kb/s per peer
> against a 200 kb/s allowance across 8, 16 and 32 peers.
>
> **A witness lane over budget is a false-positive generator, not a bandwidth
> nuisance.** At 32 peers on the old one-frame-per-send cadence the peer went
> over its 1 Mbps ceiling, the backstop shed frames and replication
> indifferently, every shed frame opened a chain hole, every hole became a
> repair on the unsheddable control lane, and observation coverage fell to 81%
> — at which point the 582 signals raised against provably honest bots were a
> statement about which frames arrived. D17 risk 3 arriving through the budget.
...
1. **Continuous cheap checks.** Every interested peer runs (a) *stateless invariant validators* on received authoritative state — speed and acceleration caps, teleport detection (displacement vs. max velocity × Δtick), fire/action rate limits, value-range checks (health ≤ max, negative-quantity rejection) — supplied by the game as `Ruleset::invariants()`; and (b) the *reconciliation-error monitor* from `orrery_predict` (D8) for entities in its own predicted set: predicted-vs-authoritative error per entity, compared against the D9 tolerance bands (ε_pos = 1 cm, ε_vel = 1 cm/s). In addition, (c) **continuous log re-execution**: members of the cell-epoch witness set (≤ 7 peers, §4.1) re-execute the streamed signed input logs for their watched core entities tick by tick — the kinematic core step costs ~µs/tick — and compare computed state against the authority's 2 Hz `StateClaim` hashes. The logs arrive in **frames covering ten ticks each, one every 167 ms**, rather than one per 20 Hz send: the cadence is derived from the share of the peer upload budget the lane may spend ([03-replication.md](03-replication.md) §5.3a), and it is deliberately slower than the escalation rule below can act on. (c) is *the* witness signal for non-predicted entities: a remote player nobody is interacting with is still audited continuously, which prediction error alone cannot provide; peers outside the witness set contribute only (a) plus (b) during interactions. (a) and (b) are O(received state); (c) adds ~µs/tick per watched entity. No extra traffic for any of them.
2. **Escalation.** A hard invariant breach, or tolerance-band violation *sustained past the 250 ms window* (not a single spike — spikes are packet loss), arms an audit. **This window is why the frame cadence is a bandwidth dial and not a detection one.** No signal is actionable until 250 ms of it has accumulated, so a stream delivered every 167 ms is already ahead of the earliest tick at which anything could be done about it; the 3 s adjudication cap is twenty times longer again. Cutting a frame per 50 ms send bought no earlier verdict and cost ~250 bytes of per-frame fixed overhead each time — measured at 32 peers, enough to put the peer over its 1 Mbps budget and collapse observation coverage to 81% ([03-replication.md](03-replication.md) §8). The observer requests the disputed window's signed input-log segment and the t₀ state claim from the suspect (witness-set members already hold the stream — D9 piggybacks logs on replication datagrams to the cell-epoch witness set, so for them this is a no-op or a gap-fill; an interacting observer outside the witness set fetches the segment over the reliable control stream), then re-executes the window headlessly in the verifiable core. Window length is bounded by the **adjudication window max: 3 s (180 ticks)** — longer anomalies are reported as consecutive windows.
3. **Discrepancy report.** On mismatch, the observer files an evidence bundle to the cluster: the signed log segment, the signed t₀ claim, the claimed vs. locally computed state hashes, and the observer's own signature. The bundle is *self-verifying* — the adjudicator needs nothing from the observer but the bundle itself. It travels as `GatewayMsg::Report` on the client's reliable control lane, the same lane intents take: a bundle is far past a datagram, and the 1 MiB reliable-message cap is what makes a 180-tick window admissible in one message. Signing is opt-in on the client — `orrery_witness` escalates only when the host supplied a `WitnessIdentity` to sign with, and shadow mode files nothing regardless (§5).
4. **Adjudication.** `orrery_persistd`'s adjudication executor routes the bundle by its pinned `RulesetId` to the matching **version-keyed sidecar worker** (the cluster retains the last 3 ruleset builds, D12) and re-runs the window. Discrete-outcome mismatch (damage, currency, loot) is a binary verdict; continuous-state deviation uses the same ε bands and sustained-error window. Failed evidence splits two ways (D10): **`EvidenceForged`** — provable fabrication, e.g. a subject signature the reporter attested as verified fails verification — strikes the *reporter*; **`Unadjudicable`** — adjudicator-side causes (ruleset build older than the 3 retained, retention miss, oversize window) — is **never a strike**; unadjudicable submissions are merely rate-limited per account. Every report is answered with a `GatewayReply::ReportVerdict`, refusals included: a gateway with no rules build linked returns `REPORT_REFUSED_NO_ADJUDICATOR` rather than silence, because a reporter that hears nothing cannot tell an operator's gap from an exoneration, and the two call for opposite responses. The rate limit is enforced at the gateway per authenticated account, not per connection — one account may hold several.
5. **Responses**, on a guilty verdict, in order: (a) **in-session authority correction** — under D32 control C4 (`--authority-correction off|shadow|live`, default `off`, with durable `ramp/authority_correction` override), the cluster revokes every offender lease through D7's existing registrar park/redistribute path, then broadcasts an `AuthorityCorrectionV1` signed by the serving gateway's already-pinned iroh identity. The payload carries the canonical adjudicated state at the last replayed tick, its `RulesetId`, and a digest binding it to the report. Delivery uses D25's interested-session set, 128-recipient cap, and shared 64-burst/32-per-second advisory bucket. A peer verifies the exact gateway signer before passing the state to its ordinary authoritative-update reconciliation ingress: rollback and replay when the tick remains in D8's 9-tick ring, otherwise authoritative snap plus the normal presentation smoothing. Shadow constructs, signs, hashes and addresses that exact payload but revokes and sends nothing; `off` does not evaluate it. (b) **durable refusal/annulment** — pending intents from the offender in the disputed window are refused, and already-journaled bulk writes are annulled by appending compensating inverse-op entries to the event journal (D11's journal is the event source; see [08-persistence.md](08-persistence.md)); (c) an **account strike** filed with `orrery_identity` (§5).
...
| Threshold (default; configurable, D33 clause (d)) | State | Effect |
|---|---|---|
| S ≥ 3.0 | **Quarantine** | All durable writes require full cluster-side validation (no attestation shortcut); ineligible as witness; telemetry sampling at 100%. Reverses as soon as read-time decay brings `S` back under 3.0. |
| S ≥ 5.0 | **Cooldown** | Identity refuses to mint or refresh a session token — no connection, durable or otherwise; D33 clause (h) grants no guest, observer or spectator experience during cooldown, and none is offered. A durable `dc` entry pins a **14-day minimum dwell** (ops dial, `cooldown_min_ms`) from the instant of entry; release requires *both* `S < 3.0` (not merely `S < 5.0`) and the full dwell elapsed, whichever comes later. An upheld appeal that re-scores the entry instant below 5.0 voids a *wrongful* cooldown's dwell outright and can release before day 14; one that leaves the entry-instant score at or above 5.0 leaves an *earned* dwell's floor intact (#1088). |
| S ≥ 7.0 | **Ban** | Account banned; bound NodeIds blacklisted. The ban is recorded as a durable row consulted *before* the score bands on every admission check, so it does not reverse when decay later brings the score back under 7.0 — a ban is terminal against time (#1059/#1061); only an administrative lift ends it. Appeal path is human review of the (self-verifying) evidence bundles. |

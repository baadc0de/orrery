...
`content/version` records `(content_build, manifest_digest, scenario_seed, config_digest, toolchain, seeded_at, universe_seed_fingerprint?)`. When it is written into the manifest file it is the **last** line, wrapped as `{"content_version": {…}}`, and that position is forced rather than chosen: `manifest_digest` covers every entry, so it does not exist until the stream has ended. A reader that wants only the inventory skips any line carrying `content_version`; a reader that wants the stamp seeks to the last line. An entry line never carries that key, so the two are told apart by shape.

**The manifest is also the deployment's shard set.** `orrery-seed shards <manifest>` reads the JSONL and prints the distinct `shard_of(cell)` — level-`SHARD_LEVEL` — cells its entries occupy, one per line as a raw `0x…` `CellId`, ready to splice into `persistd --shard`; `--json` reports the same set with the entry count and the grid, `--grid` selects one (persistd owns one grid per process). It touches no cluster, so it runs without the `fdb` feature and long after the seeding run that produced the artefact.

This exists because the alternative was measured. `persistd`'s `resolve_shards` turns an absent `--shard` into `vec![CellId::ROOT]` — one shard, one single-writer cell actor for the whole world — and the P2 kill-9 gate started every node that way. On the 10 000-entity `demo` profile (10 000 distinct interest cells over 128 shards) the effect was not a durability failure but a mailbox one: `router_apply` was 7.50 ms of a 7.81 ms acknowledged diff while `journal_commit_ms` averaged 0.46 ms, the claim phase took 58 s, and the registrar withdrew 8 921 of 10 000 leases. With the shard set derived from the manifest, the same binaries on the same box: claim phase 8.0 s, `router_apply` 1.03 ms of 1.35 ms, 532 686 durable diffs against 60 736, and 174 leases withdrawn. The collapse belongs here rather than in the harness because the manifest is this crate's format and `shard_of` is the protocol crate's function — a shell reimplementation would have to duplicate a packed-Morton `ancestor_at`, and a shard set that is subtly wrong does not fail loudly, it addresses part of the world to an actor no process owns.

**The seventh field is the universe the world belongs to.**
`universe_seed_fingerprint` is `blake3::derive_key("orrery.universe-seed-fingerprint.v1", universe_seed)` truncated to 16 bytes — a one-way, domain-separated *name* for the universe, never the seed, under the carve-out stated in [08-persistence.md](08-persistence.md) §6.1. It is supplied by `apply --universe-seed-fingerprint <hex32>` (or `ORRERY_UNIVERSE_SEED_FINGERPRINT`) and is **optional**: omit it and the world is unsealed, exactly as every world seeded before this field is. The operator passes the fingerprint rather than the seed on purpose — universe creation is where the secret lives, and a seeding host has no business holding it.

Its point is what `persistd` does with it. `--universe-seed` is a typed deployment input, and a mistyped one used to be undetectable: the adjudicator replayed against the wrong keyed RNG stream and answered confidently about a world that never existed. A `persistd` compiled with a Ruleset now fingerprints its seed, compares it against this row, and **refuses to start** — naming the flag, the row and both fingerprints — before its readiness line prints. An absent row, or a row with no fingerprint, warns and proceeds.

Because this record is also the manifest trailer, the seal **travels with the world artifact**: `verify --emit-manifest` copies the fingerprint the *cluster* holds into the trailer, so a manifest and a cluster can be cross-checked offline. That is the property a durable row of its own could not offer, and it is why the field lives here.

**The row grew a D38 trailer, not a tag.** `content/version` has an untagged predecessor — six fields of bare `postcard` — so the version is a one-byte **trailer**: `postcard(ContentVersion) ‖ 0x01`, and an absent trailer *is* v0 ([D38](adr/0038-at-rest-schema-versioning.md) clause (d)(1)). A leading tag would be read by an old binary as the length varint of `content_build` and shift every field after it. The trailer shifts nothing: an old binary's `postcard::from_bytes` discards what follows the body it decoded, so it reads the same six fields it always read and simply does not see the seal. Readers that account for every byte — which is what the shipped decoder does — refuse a v1 row as v0 rather than truncating it, and that refusal is what makes the two shapes decidable instead of guessed.

`manifest_digest` is unaffected, so **gate A4 does not move**: the digest is folded from entry encodings and the toolchain stamp, and this record is not an input to it. A golden test that pins emitted manifest *bytes* shifts once, as the paragraph below already predicts.
...
| S5b | `content/version` gains a seventh field, `universe_seed_fingerprint`, carried under a D38 one-byte trailer (absent trailer == v0), plus the §6.1 carve-out that a domain-separated one-way fingerprint of the universe seed may live in the keyspace and the seed may not | [08-persistence.md](08-persistence.md) §6, §6.1 |

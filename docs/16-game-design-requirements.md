...
- **Resurrection location is the nearest eligible clone printer (G3.6)**, and choosing it is a rule (closest, last-used, or player-selected among those with capacity). Where the clone wakes determines how far insurance must deliver and how exposed the fresh clone is. A planetside printer is a structure and can be destroyed (G4.13), so a mission's forward respawn can be taken away mid-fight: losing the printer is the surface objective that matters most, for both sides.

...
| G2.2b | **Escorts (light craft)** have **no jump capability**. They fight alongside and are moved by other means (carried, or flip-and-burn). These are the ships most players will own (G6.5). | owner mandate |
...
- **Escorts imply formation and carriage.** Either escorts ride inside a jump-capable hull (nesting, G1) or they arrive by flip-and-burn ahead of time. Both make fleet movement a planned, persisted thing. Because escorts are what most players own (G6.5), **"my ship docked inside someone else's ship" is the common case, not the edge case**: a player-owned grid nested in an organization- or mothership-owned grid, with the outer captain controlling movement and the inner owner controlling their own hull.
...
| G6.5 | **Most players do not own ships.** The economy must push the majority toward owning **light craft** (escorts, G2.2b) at most and otherwise **joining expeditions** on ships owned by others or by the mothership (G4.2). | owner mandate |

...
| `scripts/admission.py` | `/opt/orrery/bin/admission.py` | the whole script — it is the application, not a recording of it |
| `orrery-invite`, built from `crates/orrery_identity` | `/opt/orrery/bin/orrery-invite` | not tracked as a file; the operator builds the release binary and supplies it via `ORRERY_INVITE_BIN` |
...
On change it validates with `nginx -t` and only then `nginx -s reload`; for the unit it runs `systemctl daemon-reload` and prints, but never runs, `systemctl restart orrery-admission` — restarting the box office interrupts joins in flight, so that stays the operator's call. That is the rule for the two config files; the application pair below is the one exception.

**The two application files the unit runs are a coupled pair, and the installer installs them as one transaction (#1049).** `admission.py` shells out to `orrery-invite session-token` on every admission, and since #1014/#1047 that call carries `--assume-standing-good` while the binary refuses to mint without it. All four version combinations were verified against the real pre-#1047 binary, and **both mixed cells fail every admission**: the current mint refuses the flag-less call, and the pre-#1047 binary's clap rejects the unknown flag (`error: unexpected argument '--assume-standing-good' found`, exit 2). There is therefore no safe install order — installing the script first does not tolerate the old binary, which the issue filing guessed it would — so the installer enforces the coupling directly instead. The flag both halves must agree on is the version marker: the installer classifies the host's binary by whether `session-token --help` names it and the host's script by whether it passes it, refuses an `ORRERY_INVITE_BIN` that does not accept the flag (that install would be a mixed cell by construction), refuses a tracked script that no longer passes it (the pair contract broken at the source), and refuses a host script that is flag-aware but has drifted from the tracked one, with the same diff-and-reconcile rule as the config files. An old/old host upgrades in one transaction: both halves staged, both renamed into place back-to-back, script first. A host already in a mixed cell is named loudly — which cell, and that every admission is failing right now — and completed rather than refused, because completing the pair is the only remedy for it. And the one deviation from print-don't-restart: when the transaction changes either half, the installer runs `systemctl restart orrery-admission` itself, because the running service imported the script once at start but execs the binary fresh from disk on every admission — a swapped binary beside the in-memory old script is the dangerous cell with admissions live, and it lasts exactly as long as the restart is deferred. An operator run:

```
sudo ORRERY_INVITE_BIN=/path/to/target/release/orrery-invite ./scripts/deploy-web-tier.sh
```

The binary path is the one thing the installer will not guess: the repository tracks `orrery-invite`'s source, not its binary, so an unset `ORRERY_INVITE_BIN` refuses the install the way an unfilled `ORRERY_PLACEHOLDER` refuses the config files.

**The restart is verified against the running process, not assumed (#1067).** On 2026-09-04 the installer put both halves of the pair on the live host, printed `the host now runs the new pair`, and did not restart anything: the service went on running the `admission.py` it had imported 16 minutes earlier against the new binary, so every admission was refused for 33 minutes and clients saw `host_failed`. The cause was that `ORRERY_INVITE_BIN` had been read as a marker that `--self-test` was driving the run — and an operator run is *required* to set it, so every real deploy classified itself as a sandbox run and skipped its own restart. Only the `ORRERY_WEB_TIER_*` overrides mark a sandbox run now; anything added to that set must be a variable an operator run never sets. And because a wrong success line is worse than an error, the transaction no longer ends on the files: after the restart the installer asks systemd for the service's `ExecMainStartTimestampUSec` and compares it with the mtime of the `admission.py` it just installed. A process older than the script it is supposed to be running means the restart did not happen, and the install fails naming the dangerous cell it is standing in rather than printing success. `--self-test` drives that whole lane through a stand-in `systemctl` (`ORRERY_WEB_TIER_SYSTEMCTL`), so the live path — the pair installed over an already-running service, and the same install where the restart silently does nothing — is covered per-commit with no systemd, no root and no host.

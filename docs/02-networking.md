...
`gates/p4-streams-bench` measures both over real QUIC at 3% loss on a 40 ms link, across four seeds. Each direction of the trade is real and repeatable: sharing a stream between sparse control and 40 kB gap repairs costs **sparse control 2–5× its median and 3–6× its p95**, and separating them costs **the repair tail 1.4–2×**. Neither mode is faster than the other overall, and the difference between "a stream per message" and "sparse shared, bulk separate" is inside the run-to-run noise.

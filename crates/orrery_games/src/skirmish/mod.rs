...
///
/// The digest is generated by `orrery_games`' build script. It is blake3 over
/// this game crate and its metadata-resolved first-party kernel closure, with
/// comments and test-only source excluded. **Bump `version` whenever the
/// rules change** — the committed golden chains are only meaningful against
/// fixed rules, and a silent rules change would present as a determinism

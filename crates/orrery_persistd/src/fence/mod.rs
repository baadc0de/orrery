//! The `actor/{shard}` placement + fencing store (docs/08-persistence.md §3.4, §3.5).
//!
...
//! CASes `actor/{S}` from `(old_node, e)` to `(self, e+1)` in one transaction;
//! every subsequent checkpoint transaction reads `actor/{S}` and aborts if the
...
//! exactly as D11 §6 specifies (`actor/{shard_cell_id}`).
...
/// The `actor/{shard}` row: placement + fencing (D11 §6).
///
...
/// A durable placement/fencing store for `actor/{shard}` rows (D11 §3.4/§3.5).
///
...
pub trait FenceStore: Send + Sync {
    ...
    /// Retire `shard`: delete its `actor/{shard}` row (§3.5).
    ///
...
pub struct MemFenceStore {
    map: Mutex<HashMap<CellId, FenceRow>>,
}
...
impl FenceStore for MemFenceStore {
    ...
    async fn begin_split(
        ...
        let mut map = self.map.lock().expect("mem fence lock");
        let current = map.get(&parent).copied();
        ...
        for (child, row) in children {
            map.insert(*child, *row);
        }
        Ok(FenceOutcome::Fenced)
    }

    async fn retire(&self, shard: CellId) -> Result<(), FenceError> {
        self.map.lock().expect("mem fence lock").remove(&shard);
        Ok(())
    }
}
...
mod tests {
    ...
    async fn fence_from_absent_fences() {
        ...
        let new = row(1, 5);
        assert_eq!(
            store.fence(CellId::ROOT, None, &new).await.unwrap(),
            FenceOutcome::Fenced
        );
        assert_eq!(store.read(CellId::ROOT).await.unwrap(), Some(new));
    }

    #[tokio::test]
    async fn fence_conflicts_on_mismatch() {
        let store = MemFenceStore::new();
        store.fence(CellId::ROOT, None, &row(1, 5)).await.unwrap();
        // Wrong expected epoch -> conflict, row unchanged.
        let conflict = store
            .fence(CellId::ROOT, Some(&row(1, 4)), &row(2, 6))
            .await
            .unwrap();
        assert_eq!(
            conflict,
        ...
        assert_eq!(store.read(CellId::ROOT).await.unwrap(), Some(row(1, 5)));
        // Correct expected -> fences.
        assert_eq!(
            store
                .fence(CellId::ROOT, Some(&row(1, 5)), &row(2, 6))
                .await
                .unwrap(),
            FenceOutcome::Fenced
        );
        assert_eq!(store.read(CellId::ROOT).await.unwrap(), Some(row(2, 6)));
    }
    ...
    async fn begin_split_marks_parent_and_writes_children() {
        ...
        store.fence(CellId::ROOT, None, &parent_row).await.unwrap();
        ...
        assert_eq!(
            store
                .begin_split(CellId::ROOT, &parent_row, &child_rows)
                .await
                .unwrap(),
            FenceOutcome::Fenced
        );
        ...
        let parent = store.read(CellId::ROOT).await.unwrap().unwrap();
        ...
        for (child, expected) in &child_rows {
            assert_eq!(store.read(*child).await.unwrap(), Some(*expected));
        }
    }
    ...
    async fn begin_split_conflicts_on_stale_parent() {
        let store = MemFenceStore::new();
        store.fence(CellId::ROOT, None, &row(0, 3)).await.unwrap();
        // A stale expected row (epoch 2) must not split.
        let conflict = store
            .begin_split(CellId::ROOT, &row(0, 2), &[])
            .await
            .unwrap();
        assert_eq!(
            conflict,
        ...
        // Parent unchanged.
        assert_eq!(store.read(CellId::ROOT).await.unwrap(), Some(row(0, 3)));
    }
    ...
    async fn retire_removes_row() {
        let store = MemFenceStore::new();
        store.fence(CellId::ROOT, None, &row(1, 5)).await.unwrap();
        store.retire(CellId::ROOT).await.unwrap();
        assert_eq!(store.read(CellId::ROOT).await.unwrap(), None);
    }
}

...
//!
//! ```text
//! orrery-sidecar [--serve ADDR] [--seed N] [--entity ID] [--stand-in-remote ID]
//! ```
//!
//! | Flag | Effect |
//! |---|---|
...
//! | `--entity ID` | The stable id of the one entity this sidecar simulates. |
...
///
/// Hand-rolled rather than `clap`: four flags, and the workspace does not
...
fn main() -> ExitCode {
    let options = match parse(std::env::args().skip(1)) {
        ...
        Err(problem) => {
            ...
            eprintln!(
                "usage: orrery-sidecar [--serve ADDR] [--seed N] [--entity ID] [--stand-in-remote ID]"
            );
    ...
    spawn_predicted(&mut app, authority, options.entity);

...
//! # What travels
//!
//! Peers exchange two lane kinds today, and the frames here mirror them 1:1 so
//! the host-side bridge can move `(NodeId, Option<StreamMode>, Bytes)` triples
...
//!
//! Frame grammar, little-endian: `[lane u8][len u32][payload]`. Lengths beyond
...
/// `deliver` refills for in-process peers.
...
pub fn encode_frame(lane: Lane, payload: &[u8], out: &mut Vec<u8>) -> Result<(), FrameError> {
    let len = u32::try_from(payload.len()).map_err(|_| FrameError)?;
...
pub fn decode_frame(buf: &mut BytesMut) -> Result<Option<(Lane, Bytes)>, FrameError> {
    if buf.len() < 5 {
        return Ok(None);
    }
    ...
    let len = u32::from_le_bytes(buf[1..5].try_into().expect("five bytes read"));
    if len > MAX_FRAME_BYTES {
        return Err(FrameError);
    }
    let total = usize::try_from(u64::from(len) + 5).expect("bounded by MAX_FRAME_BYTES");
    if buf.len() < total {
        return Ok(None);
    }
    let mut frame = buf.split_to(total);
    let payload = frame.split_off(5);
    Ok(Some((lane, payload.freeze())))
}
...
mod tests {
    ...
    fn frames_round_trip_per_lane() {
        for (lane, payload) in [
            ...
            encode_frame(lane, payload, &mut wire).expect("in range");
            let mut buf = BytesMut::from(&wire[..]);
            let (decoded, bytes) = decode_frame(&mut buf)
                .expect("well formed")
                .expect("complete");
    ...
    fn partial_frames_wait_and_then_complete() {
        let mut wire = Vec::new();
        encode_frame(Lane::Datagram, b"hello world", &mut wire).expect("in range");
        ...
        let (_, payload) = decode_frame(&mut buf)
            .expect("well formed")
            .expect("complete");
    ...
    fn two_frames_in_one_arrival_decode_in_order() {
        let mut wire = Vec::new();
        encode_frame(Lane::StreamShared, b"first", &mut wire).expect("in range");
        encode_frame(Lane::Datagram, b"second", &mut wire).expect("in range");
        let mut buf = BytesMut::from(&wire[..]);
        assert_eq!(

...
lane_gates() {
    ...
    # The reclaimer (#781, #825) runs `rm -rf` over 100 GiB-class worktree
    # directories with no human in the loop, on the SessionEnd hook. Its
    # self-test is functional in both directions and pins the gates that make
    # that defensible: that a squash-merged lane is recognised as landed (an
    # ancestry test would report every landed lane as unmerged, and reclaim
    # nothing), that liveness is read from /proc/<pid>/cwd rather than grepped
    # out of a command line, and that a pushed-but-not-yet-merged worktree is
    # trimmed (target/ only) rather than kept.
